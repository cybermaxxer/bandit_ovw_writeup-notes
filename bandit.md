...
bandit is overthewire's entry level wargame. each level is an ssh login, solving one gets you the password for the next. there's no real exploitation here, it's linux fundamentals: file permissions, `find`/`grep` chains, encodings, cron, setuid, and a handful of git and shell escape puzzles toward the end.

this isn't just commands with no context. every level below opens with a small concept table pulled from my own study notes (what the mechanism actually is, why it applies here, what the flags mean), then the actual solve. no passwords, keys, or flags included anywhere, every level ends with "check `/etc/bandit_pass/banditn`" or equivalent, and i've redacted the value itself.
...
port 2220, not 22, bandit runs on a non standard port so it doesn't collide with the host's own ssh service or get buried in the constant bot noise hitting port 22.
## Table of Contents
...
| `user@host` syntax | tells ssh who to authenticate as and where                           | `bandit0` is the starting account, without the `user@` part ssh defaults to your local machine's username, which doesn't exist remotely |
...
first connection throws a host key prompt, type `yes`, then the password (`bandit0`, given on the site). terminal shows nothing while you type it, that's intentional, not broken. once in:
...
is `./` a real mechanism or convention? real: `.` is always an alias for the current directory, `/` separates it from what follows, it's a literal path, not a trick.

is `--` universal? it's a widely followed posix convention across most cli tools, not a hard guarantee for every program, but safe to reach for by default.
...
`cat *` prints everything in the current directory.
if you want to be more precise you could do cat -- - (-- means skip flags, say -t in ss -t)
...
is the dot for hidden a real filesystem attribute? no, pure display convention, `ls` and file managers respect it, the filesystem itself doesn't treat dotfiles any differently at the permission level.
...
is `file` guessing from the name? no, real mechanism, it reads actual bytes and matches them against a signature database, filenames are irrelevant to it.
...
is `c` for bytes derivable or memorized? memorized, it's `find`'s own unit vocabulary (`c` = bytes, `k` = kb, `m` = mb), not derived from anything. skip the suffix and `find` defaults to 512 byte blocks, giving silently wrong matches.

multiple predicates in `find` are anded together automatically, no explicit `-a` needed.
...
|flag|meaning|why used here|
|---|---|---|
|`find /`|search starting at filesystem root|the target isn't under your home directory at all this time, it's anywhere on the box|
...
| ------------ | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `sort`       | reorders lines alphabetically                            | `uniq` only compares adjacent lines, sorting is what groups duplicates together so it can catch them |
...
| concept                      | explanation                                                                            | why it matters here                                                                                                         |
| ---------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| rot13                        | shifts each letter 13 places through the alphabet, wrapping around                     | a is n, b is o, etc, 13 is exactly half of 26                                                                               |
...
---
this was rather tricky. so the way it works is that you enter a set1. it has, say x characters. in another set, you need to enter set2, that could consist of z sets that amount to x characters in total. 
e.g in our case, our first to-be-translated set is A-Z that amounts to 26 chars, and the second one is 26 chars too. 
that means that we can enter up to 52 total character mappings, and the moment one of the sets in set 2 category reaches 26 chars, the following a-z set will start to be getting mapped.
that being said, A-Z(26) maps to N-Z(13) (A-N B-O...), the when we reach N in our first set, N will map to A and so on. after having finished parsing it, we will do the same thing to a-z.
...
is nmap's service detection guaranteed? not perfectly, it makes educated guesses off banners/behavior, verifying manually with `openssl s_client` (and falling back to `nc` if that fails) is still necessary.
...
| concept               | explanation                                                                               | why it matters here                                                                  |
| --------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| `diff -u file1 file2` | compares two files, outputs the lines that differ<br>add -u for github style highlighting | algorithmically finds the one changed line instead of you eyeballing two large files |
...
| concept                   | explanation                                                                                                                                                                                                                                                                                    | why it matters here                                                |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| setuid bit                | a permission bit making a binary run with the file owner's privileges, not the caller's.<br>in other words, no matter who you are, if the file is setuid'ed you will run it as somebody who's created it.<br>incredibly dangerous and infamous, especially with stuff created by the root user | the entire mechanism of the level, the binary is owned by bandit20 |
...
is setuid a real os mechanism or convention? real, an actual permission bit enforced by the kernel at execution time, not a naming trick.
...
| & at the end                    | runs the thing and puts it background mode, essentially giving you back the command prompt       | well you have to both be a host and the server                                         |
| fg (x)                          | x is the job you want to put back to the interactive mode                                        | you have to write something back                                                       |
...
| concept       | explanation                                                                                                          | why it matters here                                                                                                                                                                                       |
| ------------- | -------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| brute force   | trying every possible value in a keyspace until one works                                                            | the only viable approach, the pin isn't derivable or stored anywhere readable                                                                                                                             |
...
| for loop      | do something for specific number of times, here we use c-like for loop to iterate over every number in range 0-10000 | we need it to iterate through every combination                                                                                                                                                           |
| printf        | formatted printing                                                                                                   | we use it in order to provide the expected combination using %04d<br>we may specify that we need exactly  4 numbers in each output resulting to expected combination as following:<br>0000, 0001, 0002... |
| \| ncat       | piping                                                                                                               | whatever the for loop outputs here, must go directly to ncat. that way i didn't have to write a script.                                                                                                   |


**solve**

a small python script handling one persistent socket, since a clean pure-bash version gets messy fast:
...
|why uppercasing `$0` doesn't break it|the wrapper mangles typed characters, `$0` is a variable expansion resolved by the shell itself|the actual gap being exploited, not a typed command getting mangled, a variable expansion the wrapper never accounted for|
